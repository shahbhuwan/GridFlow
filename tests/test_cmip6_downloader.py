...
import gridflow.download.cmip6_downloader as cmip6_downloader
...
def test_fetch_datasets_multiple_nodes_unique(mocker, query_handler):
    ...
    assert len(files) == 1  # Only from first node
    assert files[0]['id'] == '1'
...
def test_create_download_session_dry_run(mocker, mock_stop_event, caplog):
    ...
    create_download_session({'project': 'CMIP6'}, settings, mock_stop_event)
...
def test_create_download_session_success(mocker, mock_stop_event):
    ...
    create_download_session({'project': 'CMIP6'}, settings, mock_stop_event)
    assert mock_fm.return_value.save_metadata.call_count == 2  # to_download and failed
...
def test_create_download_session_exception(mocker, mock_stop_event, caplog):
    ...
    create_download_session({'project': 'CMIP6'}, {}, mock_stop_event)
...
def test_main_success(mock_signal, mock_session, mock_logging, mock_parser):
    mock_args = MagicMock(config=None, demo=False, project='CMIP6', model='HadGEM3-GC31-LL', log_dir='./logs', log_level='info')
...
@patch('argparse.ArgumentParser')
@patch('gridflow.download.cmip6_downloader.setup_logging')
@patch('gridflow.download.cmip6_downloader.stop_event')
@patch('gridflow.download.cmip6_downloader.create_download_session')
